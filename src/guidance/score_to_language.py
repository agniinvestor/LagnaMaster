...
_TIERS = [
    (3.0,  5, "Clear passage",         "Strong foundations support action in this area.",               "affirming"),
    (1.5,  4, "Favourable",            "Conditions are generally supportive of your efforts here.",      "encouraging"),
    (0.5,  3, "Mixed — lean in",       "Some supporting factors are present; preparation will help.",   "balanced"),
    (-0.5, 2, "Neutral",               "Signals are mixed. Neither strongly supported nor obstructed.", "neutral"),
    (-1.5, 1, "Navigate carefully",    "Some friction is present. Patience tends to work better than urgency.", "cautious"),
]
_FLOOR = (None, 0, "Significant resistance",
          "This area carries meaningful challenges. Building your foundation is wiser than forcing outcomes.", "grounded")
...
def score_to_signal(score: float) -> SignalLevel:
    ...
    for threshold, bars, label, template, tone in _TIERS:
        if score >= threshold:
            return SignalLevel(bars=bars, timing_label=label,
                               l1_template=template, guidance_tone=tone)
...
def domain_l1_sentence(score: float, domain: str, promise_strength: str = "Moderate",
                        timing: str = "Future") -> str:
    """
