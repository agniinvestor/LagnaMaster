...
class L3Content:
    ...
    disclaimer: str = (
        "Advanced technical view — for practitioners and advanced students."
    )
...
def build_l1(
    domain: str, score: float, promise_strength: str, timing: str, confidence_label: str
) -> L1Content:
...
def build_l2(
    ...
    if transit_supported:
        ...
        factors.append(
            "Period is active but transit support is limited — action is possible but patience helps."
        )
    ...
    if not factors:
        factors.append(
            f"Multiple chart layers were evaluated for {domain.replace('_', ' ')}."
        )
    ...
    return L2Content(
        factors=factors, timing_note=timing_note, domain_context=domain_ctx
    )
...
def explain(
    ...
    active_dasha: str = "",
    dasha_activated: bool = False,
    transit_supported: bool = False,
    dominant_theme: str = "",
    lpi_data: dict | None = None,
    ...
    if depth == "L3":
        l3 = build_l3(score, lpi_data or {}, rules_fired or [], confidence_data or {})

    return GuidanceContent(domain=domain, l1=l1, l2=l2, l3=l3, depth_requested=depth)
