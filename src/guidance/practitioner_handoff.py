...
def should_recommend_practitioner(requires_expert_review: bool,
                                   critical_exceptions: int,
                                   confidence_uncertain_houses: int) -> bool:
    """Determine if practitioner referral is warranted."""
    return (requires_expert_review or
            critical_exceptions > 0 or
            confidence_uncertain_houses >= 4)


def build_chart_summary(chart, dashas=None, on_date=None,
                         confidence_report=None) -> ChartSummary:
    ...
    try:
        ...
        for st in compute_stelliums(chart):
            if len(st.planets) >= 3:
                notable.append(f"{len(st.planets)}-planet {st.nature.lower()} stellium "
    ...
    if confidence_report:
        uncertain = [h for h, hc in confidence_report.houses.items()
    ...
    return ChartSummary(
        ...
        active_dasha=active_md, active_antardasha=active_ad,
        notable_features=notable, confidence_notes=conf_notes,
        practitioner_note=(
