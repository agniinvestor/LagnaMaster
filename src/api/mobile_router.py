...
async def mobile_guidance(chart_id: str, domain: str):
    ...
    return GuidanceSummaryResponse(
        domain=domain, heading=f"{domain.title()} — {sig.timing_label}",
        summary=sig.l1_template, signal_bars=sig.bars,
        signal_display=signal_bars_display(sig.bars),
        timing_label=sig.timing_label, confidence_label="Moderate",
...
async def schedule_timing_alert(req: TimingAlertRequest):
    ...
    return TimingAlertResponse(
        ...
        domain=req.domain, alert_date=req.alert_date,
        heading=f"{req.domain.title()} — {sig.timing_label}",
        summary=sig.l1_template, signal_bars=sig.bars,
        timing_label=sig.timing_label, disclaimer=get_disclaimer(req.domain),
    )
