...
class UserOut(BaseModel): id: int; username: str; email: str; created_at: str; is_active: bool
...
def get_current_user(creds: HTTPAuthorizationCredentials = Depends(_b)) -> A.UserRecord:
    ...
    try: uid = A.verify_access_token(creds.credentials)
    except ValueError as e: raise HTTPException(401, str(e), headers={"WWW-Authenticate":"Bearer"})
    user = A.get_user_by_id(uid)
    if user is None or not user.is_active: raise HTTPException(401, "User not found or deactivated")
    return user

def _tout(pair): return TokOut(access_token=pair.access_token, refresh_token=pair.refresh_token,
                                token_type=pair.token_type, access_expires_in=pair.access_expires_in,
                                refresh_expires_in=pair.refresh_expires_in)
def _uout(u): return UserOut(id=u.id,username=u.username,email=u.email,created_at=u.created_at,is_active=u.is_active)

@router.post("/register", response_model=UserOut, status_code=201)
def register(req: RegReq):
    try: A.init_user_db(); return _uout(A.register_user(req.username,req.email,req.password))
...
def login(req: LoginReq):
    ...
    if u is None: raise HTTPException(401, "Invalid username or password")
    return _tout(A.create_token_pair(u.id))

@router.post("/refresh", response_model=TokOut)
def refresh(req: RefreshReq):
    try: uid = A.verify_refresh_token(req.refresh_token)
    except ValueError as e: raise HTTPException(401, str(e))
    u = A.get_user_by_id(uid)
    if u is None or not u.is_active: raise HTTPException(401, "User not found or deactivated")
    return _tout(A.create_token_pair(uid))

@router.get("/me", response_model=UserOut)
def me(u: A.UserRecord = Depends(get_current_user)): return _uout(u)
