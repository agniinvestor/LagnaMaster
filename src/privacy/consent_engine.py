...
def grant_consent(user_id: str, purpose: str, jurisdiction: str = "default",
                   db_path: str | Path = "lagna.db") -> ConsentRecord:
    ...
    return ConsentRecord(user_id=user_id, purpose=purpose, granted_at=datetime.utcnow(),
                          withdrawn_at=None, jurisdiction=jurisdiction,
                          version=CONSENT_VERSION, is_active=True)


def withdraw_consent(user_id: str, purpose: str,
                      db_path: str | Path = "lagna.db") -> bool:
    ensure_consent_tables(db_path)
    now = datetime.utcnow().isoformat()
    with _get_conn(db_path) as conn:
        conn.execute("""
...
def has_active_consent(user_id: str, purpose: str = "core",
                        db_path: str | Path = "lagna.db") -> bool:
    ensure_consent_tables(db_path)
    with _get_conn(db_path) as conn:
        row = conn.execute("""
...
def right_to_erasure(user_id: str, db_path: str | Path = "lagna.db") -> dict:
    ...
    with _get_conn(db_path) as conn:
        ...
        for table, col in erasure_targets:
            try:
                n = conn.execute(f"DELETE FROM {table} WHERE {col}=?", (user_id,)).rowcount
                if n > 0:
    ...
    return {"erased": True, "tables_cleared": cleared,
            "tombstone": True, "gdpr_article": "17"}
