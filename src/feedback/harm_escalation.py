...
def check_usage_pattern(domain_views_today: dict[str, int],
                         session_count_today: int,
                         session_count_week: int,
                         recent_ratings: list[str]) -> EscalationSignal:
    """
    ...
    if neg_domain_count >= _CONCERN_REPEAT_THRESHOLD:
        return EscalationSignal(
            triggered=True, severity="gentle", show_to_user=True,
            prompt=(
    ...
    if concerning_count >= 2:
        return EscalationSignal(
            triggered=True, severity="moderate", show_to_user=True,
            prompt=(
    ...
    return EscalationSignal(triggered=False, severity="none",
                             prompt="", show_to_user=False)
