...
def log_session(user_id: str, domain: str = "general",
...
def check_dependency_status(user_id: str,
    ...
    nudge = ""
    if is_overuse:
        nudge = ("You're checking in very frequently. For your wellbeing, "
        ...
        nudge = ("Guidance works best as one input among many. "
                 "Your own judgment and trusted advisors are always the primary source.")

    return DependencyStatus(
        sessions_today=today_count, sessions_week=week_count,
        show_nudge=show_nudge, nudge_text=nudge, is_overuse=is_overuse,
    )
