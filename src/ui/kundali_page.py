...
def _birth_inputs(prefix: str, label: str, demo_defaults: dict) -> dict | None:
    ...
    with col1:
        bd = st.date_input(f"Birth date ({prefix})",
                           value=demo_defaults["date"],
                           min_value=date(1915, 1, 1),
                           key=f"{prefix}_date")
        hour = st.slider("Hour", 0, 23, demo_defaults["hour"], key=f"{prefix}_hour")
        minute = st.slider("Minute", 0, 59, demo_defaults["minute"], key=f"{prefix}_minute")
    with col2:
        lat = st.number_input("Latitude (°N)", -90.0, 90.0,
                              demo_defaults["lat"], 0.001, key=f"{prefix}_lat")
        lon = st.number_input("Longitude (°E)", -180.0, 180.0,
                              demo_defaults["lon"], 0.001, key=f"{prefix}_lon")
        tz = st.number_input("UTC offset", -12.0, 14.0,
                             demo_defaults["tz"], 0.5, key=f"{prefix}_tz")
        ayanamsha = st.selectbox("Ayanamsha", ["lahiri", "raman", "krishnamurti"],
                                 key=f"{prefix}_ayan")
    return {
        "year": bd.year, "month": bd.month, "day": bd.day,
        "hour": hour + minute / 60,
        "lat": lat, "lon": lon, "tz_offset": tz, "ayanamsha": ayanamsha,
    }
...
def render_kundali_tab() -> None:
    ...
    demo_m = dict(date=date(1889, 11, 14), hour=23, minute=36,
                  lat=27.1767, lon=79.9464, tz=5.5)   # Allahabad
    demo_f = dict(date=date(1901, 11, 28), hour=22, minute=0,
                  lat=51.5074, lon=-0.1278, tz=0.0)   # London
    ...
    with col_m:
        params_m = _birth_inputs(
            "male", "Chart A (Male / Person 1)",
            demo_m if demo else dict(date=date(1990, 1, 1),
                                     hour=6, minute=0,
                                     lat=28.6139, lon=77.209, tz=5.5),
        )
    with col_f:
        params_f = _birth_inputs(
            "female", "Chart B (Female / Person 2)",
            demo_f if demo else dict(date=date(1992, 6, 15),
                                     hour=10, minute=0,
                                     lat=19.076, lon=72.878, tz=5.5),
        )
...
def _render_result(
    ...
    with c2:
        st.metric("Total Gunas", f"{result.total_score} / 36",
    ...
    with c3:
        ...
        st.metric("Mangal Dosha (B)", "Yes ⚠️" if result.mangal_dosha_female else "No ✅")
    ...
    with st.expander("🗺️ View Birth Charts Side-by-Side"):
        ...
        with c1:
            st.caption("Chart A")
            st.markdown(south_indian_svg(chart_m, name="Chart A"),
                        unsafe_allow_html=True)
        with c2:
            st.caption("Chart B")
            st.markdown(south_indian_svg(chart_f, name="Chart B"),
                        unsafe_allow_html=True)
