...
def _worker(args):
    ...
    if bdi:
        try:
            ...
            ds = compute_vimshottari_dasa(c, _d.fromisoformat(bdi))
...
def compute_sensitivity(
    ...
    wh = window_minutes / 60.0
    sh = [max(0.0, min(23.9999, hour + rng.uniform(-wh, wh))) for _ in range(n_samples)]
    bdi = birth_date.isoformat() if birth_date else None
    al = [
        (year, month, day, h, lat, lon, tz_offset, ayanamsha, ephe_path, bdi)
        for h in sh
    ]
    orig = _worker(
        (year, month, day, hour, lat, lon, tz_offset, ayanamsha, ephe_path, bdi)
    )
    results = []
    ...
    lc = {}
    for r in results:
        lc[r["lagna_sign"]] = lc.get(r["lagna_sign"], 0) + 1
    dl = max(lc, key=lambda k: lc[k])
    n = len(results)
    ls = lc.get(orig["lagna_sign"], 0) / n if n else 1.0
    mc = {}
    for r in results:
        mc[r["md_lord"]] = mc.get(r["md_lord"], 0) + 1
    dm = max(mc, key=lambda k: mc[k]) if mc else "N/A"
    ds = (
        mc.get(orig["md_lord"], 0) / n
        if birth_date and orig["md_lord"] != "N/A" and n
        else 1.0
    )
    houses = {}
    for h in range(1, 13):
        hs = [r["house_scores"].get(h, 0.0) for r in results]
        hr = [r["house_ratings"].get(h, "?") for r in results]
        hn = len(hs)
        mean = sum(hs) / hn if hn else 0.0
        std = statistics.stdev(hs) if hn > 1 else 0.0
        mn = min(hs) if hs else 0.0
        mx = max(hs) if hs else 0.0
        houses[h] = HouseSensitivity(
            ...
            round(mx, 4),
            round(mx - mn, 4),
            max(set(hr), key=hr.count) if hr else "?",
            std < 0.5,
        )
    return SensitivityReport(
        n_samples, window_minutes, round(ls, 4), dl, round(ds, 4), dm, houses
    )
