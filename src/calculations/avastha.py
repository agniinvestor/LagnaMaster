...
DEEPTADI_STATES = {
    "Deepta": {
        "description": "Exalted/Moolatrikona — fully illumined",
        "multiplier": 1.5,
    },
...
def compute_lajjitadi(chart) -> LajjitadiResult:
    ...
    triggers = []
    if in_dusthana:
        triggers.append(f"in H{fifth_lord_house} (dusthana)")
    if combust:
        triggers.append("combust")
    if nodes_present:
        triggers.append("conjunct Rahu/Ketu")
    if malefic_present:
        triggers.append(
            f"conjunct malefic(s): {[p for p in co_signs if p in nat_malefics]}"
        )
...
def compute_all_avasthas(chart) -> AvasthaReport:
    ...
    effective = {
        p: DEEPTADI_STATES[deeptadi[p]]["multiplier"]
        * BALADI_STATES[baladi[p]]["multiplier"]
        for p in _PLANETS_7
    }
