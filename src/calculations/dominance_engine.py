...
class DominanceFactor:
    factor_type: str      # "Benefic Override"/"Malefic Dominance"/"Dasha Priority"/"Activation"
    planet: str
...
class DominanceReport:
    ...
    global_tone: str   # "Positive"/"Mixed Positive"/"Neutral"/"Mixed Negative"/"Negative"


def compute_dominance_factors(chart, dashas=None,
                               on_date: date | None = None) -> DominanceReport:
    if on_date is None:
        on_date = date.today()
    ...
    if jup_h in _KENDRA:
        ...
        jup_aspects = {jup_h, (jup_h+4)%12+1, (jup_h+6)%12+1, (jup_h+8)%12+1}
    ...
    if avg_score > 1.5:   tone = "Positive"
    elif avg_score > 0.5:  tone = "Mixed Positive"
    elif avg_score > -0.5: tone = "Neutral"
    elif avg_score > -1.5: tone = "Mixed Negative"
    else:                  tone = "Negative"
