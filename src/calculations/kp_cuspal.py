...
def compute_cuspal_sub_lords(chart) -> dict[int, CuspalSubLord]:
    ...
    for house in range(1, 13):
        ...
        try:
            sig = compute_kp_significators(
                entry.sub_lord,
                chart.planets.get(
                    entry.sub_lord, type("", (), {"longitude": 0.0})()
                ).longitude
                if entry.sub_lord in chart.planets
                else 0.0,
                chart,
            )
...
def compute_kp_event_promise(
    cuspal_sub_lords: dict[int, CuspalSubLord],
) -> dict[str, dict]:
    """
...
def compute_kp_analysis(chart, query_datetime=None) -> KPAnalysisResult:
    ...
    if promised_events and ruling:
        ...
        for csl in cuspal_sub_lords.values():
            if (
                csl.has_promise
                and csl.sub_lord in ruling
                and csl.sub_lord not in fructification
            ):
