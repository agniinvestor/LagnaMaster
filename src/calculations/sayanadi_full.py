...
def compute_sayanadi_full(
    planet: str, chart, yuddha_losers: set[str] | None = None
) -> SayanadiResult:
    """
    ...
    if yuddha_losers and planet in yuddha_losers:
        return SayanadiResult(planet, "Deena", 0.50, "Defeated in Graha Yuddha")

    # ── Priority 3: Sthira (own/exaltation sign) ──────────────────────────────
    if si in _OWN.get(planet, set()) or _EXALT_SI.get(planet) == si:
        return SayanadiResult(planet, "Sthira", 1.25, "In own or exaltation sign")
    ...
    if is_friendly and _has_benefic_aspect(planet, si, chart):
        return SayanadiResult(
            planet, "Mudita", 1.25, f"Friendly sign ({lord_name}) + benefic aspect"
        )
    ...
    if is_odd:
        if decan == 0:
            return SayanadiResult(
                planet, "Sayana", 0.60, "Odd sign, 1st decanate (0°–10°) — sleeping"
            )
        elif decan == 1:
            return SayanadiResult(
                planet, "Upavesh", 0.75, "Odd sign, 2nd decanate (10°–20°) — stirring"
            )
        else:
            return SayanadiResult(
                planet,
        ...
        if decan == 0:
            return SayanadiResult(
                planet, "Kautuka", 0.85, "Even sign, 1st decanate (0°–10°) — playful"
            )
        elif decan == 1:
            return SayanadiResult(
                planet,
...
def compute_all_sayanadi(
    chart, yuddha_losers: set[str] | None = None
