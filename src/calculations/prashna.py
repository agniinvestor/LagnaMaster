...
def analyze_prashna(
    chart, query_type: str = "general", query_dt: datetime | None = None
) -> PrashnaAnalysis:
    """
    ...
    if not reasoning:
        reasoning.append(
            f"Moon in {moon_nak_name} nakshatra — general indicator for {query_type} query"
        )
    ...
    return PrashnaAnalysis(
        ...
        hora_lord=hora_lord,
        hora_quality=hora_quality,
        moon_nakshatra=moon_nak_name,
        moon_house=moon_house,
        key_house=key_house,
        key_house_lord=key_house_lord,
        key_house_score=round(key_score, 3),
