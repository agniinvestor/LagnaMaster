...
def compute_panchanga(
    sun_lon: float, moon_lon: float, dt: datetime | None = None
) -> Panchanga:
    """
    ...
    auspicious = (
        amrita
        or sarva
        or tithi_raw in {2, 3, 5, 7, 10, 11, 13, 15}
        and vara_jyotish in {1, 3, 4, 5}
    )
    ...
    return Panchanga(
        ...
        amrita_siddhi=amrita,
        sarvaartha_siddhi=sarva,
...
def compute_hora(dt: datetime, sunrise_hour: float = 6.0) -> tuple[str, int]:
    ...
    start_idx = [0, 3, 6, 2, 5, 1, 4][
        vara_jyotish
    ]  # Sun→0,Moon→3,Mars→6,Mer→2,Jup→5,Ven→1,Sat→4
    hora_lord_idx = (start_idx + hora_num) % 7
    return _HORA_SEQ[hora_lord_idx], hora_num + 1


def compute_choghadiya(
    dt: datetime, sunrise_hour: float = 6.0, sunset_hour: float = 18.0
) -> dict:
    """
