...
def _same_sign(chart, p1: str, p2: str) -> bool:
    return (_planet_sign_index(chart, p1) == _planet_sign_index(chart, p2)
...
def detect_raja_dhana_yogas(chart, dashas=None, on_date=None) -> list[YogaResult]:
    ...
    for h_kendra, h_trikona, label in raja_pairs:
        ...
        active_p = lord_k if _is_dasha_active(lord_k, dashas, on_date) else (
                   lord_t if _is_dasha_active(lord_t, dashas, on_date) else None)
    ...
    # 5 Dhana Yoga pairs
    dhana_pairs = [
        (2,11,"DY: H2+H11"),(2,9,"DY: H2+H9"),
        (2,5,"DY: H2+H5"),(11,9,"DY: H11+H9"),
        (11,5,"DY: H11+H5"),
    ]
    for ha, hb, label in dhana_pairs:
        ...
        dw = 1.0 if (_is_dasha_active(la, dashas, on_date) or
                     _is_dasha_active(lb, dashas, on_date)) else 0.5
        score = 2.0 if present else 0.0
        _desc = {"DY: H2+H11":"Classic Dhana Yoga","DY: H2+H9":"Wealth via fortune",
                 "DY: H2+H5":"Wealth via intellect","DY: H11+H9":"Gains via dharma",
...
def detect_viparita_yogas(chart, dashas=None, on_date=None) -> list[YogaResult]:
    ...
    viparita = [
        ("Harsha Yoga",  6, "H6 lord in H6/H8/H12 → victory over enemies",    "BPHS Ch.42 v.3-4"),
        ("Sarala Yoga",  8, "H8 lord in H6/H8/H12 → long life, prosperous",   "BPHS Ch.42 v.5-6"),
        ("Vimala Yoga", 12, "H12 lord in H6/H8/H12 → virtuous, free enemies",  "BPHS Ch.42 v.7-8"),
    ]
    for name, src_house, desc, src in viparita:
        ...
        score = 2.0 if present else 0.0
        results.append(YogaResult(
            name=name, yoga_type="Viparita",
            planets=[lord], present=present,
            score=score, dasha_weight=dw,
            weighted_score=round(score * dw, 2),
    ...
    l6 = hmap.house_lord[5]; l8 = hmap.house_lord[7]
    l6_h = hmap.planet_house.get(l6, 0); l8_h = hmap.planet_house.get(l8, 0)
    dainya = (l6_h == 8 and l8_h == 6)
    dw = 1.0 if (_is_dasha_active(l6, dashas, on_date) or
    ...
    for p in planets_7:
        ...
        if pos.sign_index != debil_si:
            results.append(YogaResult(
                ...
                planets=[p], present=False, score=0.0,
                dasha_weight=0.5, weighted_score=0.0,
        ...
        results.append(YogaResult(
            ...
            description=(f"Neecha Bhanga: cond1={cond1} cond2={cond2} d9={cond3}"
                         if present else f"{p} debilitated, no cancellation"),
...
class RasiDrishtiMap:
    ...
    house_aspects: dict[int, dict]   # house -> {aspecting_signs, benefic_planets, malefic_planets, net}
...
def compute_rasi_drishti(chart) -> RasiDrishtiMap:
    ...
    for h in range(1, 13):
        house_si = (lsi + h - 1) % 12
        aspecting_signs = [si for si in range(12) if house_si in _RASI_DRISHTI.get(si, set())]
        benefics = []
        malefics = []
        for asi in aspecting_signs:
            for p in sign_pl.get(asi, []):
                if p in _NAT_BENEFIC: benefics.append(p)
                if p in _NAT_MALEFIC: malefics.append(p)
