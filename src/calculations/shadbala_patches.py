...
def extract_nbry_yogas(chart) -> list[dict]:
    ...
    for planet, d in dignities.items():
        if d.dignity == DignityLevel.NEECHA_BHANGA_RAJA:
            conditions_met = []
            if d.nb_lord_kendra_lagna:
                conditions_met.append("Debil lord in Kendra from Lagna")
            if d.nb_lord_kendra_moon:
                conditions_met.append("Debil lord in Kendra from Moon")
            if d.nb_exalt_kendra_lagna:
                conditions_met.append("Exalt lord of debil sign in Kendra from Lagna")
            if d.nb_exalt_kendra_moon:
                conditions_met.append("Exalt lord in Kendra from Moon")
            if d.nb_aspected_by_lord:
                conditions_met.append("Debil lord aspects planet")
            if d.nb_parivartana:
                conditions_met.append("Parivartana with sign lord")
