...
_STHIR_KARAK = {
    ...
    6:["Mars","Saturn"], 7:["Venus"], 8:["Saturn"], 9:["Jupiter"],
...
class ChartScoresV2:
    ...
    def summary(self) -> str:
        lines = [f"LagnaMaster v2 Scores — {self.lagna_sign} Lagna (engine {self.engine_version})"]
        for h, hs in sorted(self.houses.items()):
            lines.append(f"  H{h:2d} {hs.domain:25s} {hs.final_score:+6.2f}  {hs.rating}")
        return "\n".join(lines)
...
def score_chart_v2(chart) -> ChartScoresV2:
    ...
    from src.calculations.house_lord import compute_house_map, is_kendra, is_trikona, is_dusthana
    ...
    def aspects(planet: str, target_house: int) -> bool:
        ...
        if diff == 6: return True  # 7th aspect all planets
    ...
    for house in range(1, 13):
        ...
        in_house = [p for p, pp in chart.planets.items() if pp.sign_index == (chart.lagna_sign_index + house - 1) % 12]

        results: list[RuleResultV2] = []

        def rule(code, desc, score, wc=False, triggered=True):
            results.append(RuleResultV2(code, desc, score if triggered else 0.0, wc, triggered))
        ...
        fb_in = [p for p in in_house if is_func_benefic(p) and p not in ("Rahu","Ketu")]
        if fb_in:
        ...
        fb_aspects = [p for p in chart.planets if is_func_benefic(p) and aspects(p, house) and p not in in_house]
        if fb_aspects:
        ...
        rule("R04", "Bhavesh in Kendra/Trikona", 2.0, triggered=is_kendra(bh_house) or is_trikona(bh_house))

        # R05 (WC) bhavesh with kendra/trikona lord
        bh_cotenants = [p for p, pp in chart.planets.items()
                        if pp.sign_index == chart.planets.get(bhavesh, None) and p != bhavesh
                        if chart.planets.get(bhavesh)] if bhavesh in chart.planets else []
        kt_lords = {hmap.house_lord[h-1] for h in [1,4,5,7,9,10]}
        rule("R05", "Bhavesh with Kendra/Trikona lord (WC)", 0.5, wc=True,
             triggered=bool(set(bh_cotenants) & kt_lords))

        # R06 bhavesh with func benefic
        bh_with_fb = [p for p in bh_cotenants if is_func_benefic(p)] if bh_cotenants else []
        if bh_with_fb:
        ...
        if fb_asp_bh:
            ...
            rule("R07", "Func. benefic aspects bhavesh sign (WC)", 0, wc=True, triggered=False)

        # R09 func malefic in house
        fm_in = [p for p in in_house if is_func_malefic(p)]
        rule("R09", "Func. malefic in house", -1.0 * len(fm_in) if fm_in else 0, triggered=bool(fm_in))

        # R10 func malefic aspects house
        fm_asp = [p for p in chart.planets if is_func_malefic(p) and aspects(p, house) and p not in in_house]
        ...
        bh_with_fm = [p for p in bh_cotenants if is_func_malefic(p)] if bh_cotenants else []
        rule("R13", "Bhavesh with func. malefic", -1.0, triggered=bool(bh_with_fm))

        # R14 (WC) func malefic aspects bhavesh sign
        fm_asp_bh = [p for p in chart.planets if is_func_malefic(p) and aspects(p, bh_h_from_lagna) and p != bhavesh]
        rule("R14", "Func. malefic aspects bhavesh sign (WC)", -0.5, wc=True, triggered=bool(fm_asp_bh))

        # R15 bhavesh in dusthana
        rule("R15", "Bhavesh in Dusthana", -2.0, triggered=is_dusthana(bh_house))

        # R16 bhavesh with dusthana lord
        bh_with_dl = [p for p in bh_cotenants if p in roles.dusthana_lords] if bh_cotenants else []
        ...
        rule("R20", "Bhavesh in Dig Bala house", 0.5,
        ...
        houses_out[house] = HouseScoreV2(
            house=house, domain=_DOMAINS[house],
            bhavesh=bhavesh, bhavesh_house=bh_house,
            rules=results, raw_score=round(raw, 3),
            final_score=round(final, 2), rating=_rating(final),
