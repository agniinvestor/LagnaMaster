...
def compute_kala_sarpa(chart) -> KalaSarpaResult:
    ...
    disclaimer = (
        "Kala Sarpa Yoga is a modern practitioner convention, "
    ...
    if not rahu_pos or not ketu_pos:
        return KalaSarpaResult(
            ...
            planets_outside=[],
            is_reverse=False,
            classical_disclaimer=disclaimer,
    ...
    if present:
        interp = (
            f"{'Kala Amrita (reverse) Yoga' if is_reverse else 'Kala Sarpa Yoga'} — "
        ...
        interp = (
            f"Partial Kala Sarpa — {count} of 7 planets inside the axis. "
            "Mild influence by convention. Not classical."
        )
    else:
        interp = (
            "Kala Sarpa Yoga not present — planets distributed across both hemispheres."
        )
