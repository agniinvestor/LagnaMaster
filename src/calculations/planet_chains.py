...
def compute_stelliums(chart) -> list[Stellium]:
    ...
    for si, planets in by_sign.items():
        ...
        if len(bens) > len(mals):   nature = "Benefic"
        elif len(mals) > len(bens): nature = "Malefic"
        else:                       nature = "Mixed"
        ...
        interp = (f"{len(planets)}-planet {nature.lower()} stellium in "
                  f"{_SIGNS[si]} (H{house}): {planets}. "
                  + ("Concentrated power, strong house activation." if nature == "Benefic"
        ...
        stelliums.append(Stellium(
            sign_index=si, sign_name=_SIGNS[si], planets=planets,
            benefic_count=len(bens), malefic_count=len(mals),
            nature=nature, house=house, interpretation=interp,
        ))

    return stelliums


def compute_dispositor_chain(planet: str, chart, visited: set | None = None) -> DispositorChain:
    ...
    return DispositorChain(
        start_planet=planet, chain=chain, final_dispositor=final,
        chain_length=len(chain), is_self_disposed=is_self,
        is_mutual=is_mutual, interpretation=interp,
    )
...
def compute_all_dispositor_chains(chart) -> dict[str, DispositorChain]:
    ...
    return {p: compute_dispositor_chain(p, chart) for p in planets_9
    ...
    for p1 in planets_7:
        for p2 in planets_7:
            ...
            if lord1 == p2 and lord2 == p1:
                ...
                _OWN = {"Sun":{4},"Moon":{3},"Mars":{0,7},"Mercury":{2,5},
                        "Jupiter":{8,11},"Venus":{1,6},"Saturn":{9,10}}
                strength = "Strong" if (pos1.sign_index in _OWN.get(p2,set()) and
                                         pos2.sign_index in _OWN.get(p1,set())) else "Partial"
                receptions.append(MutualReception(
                    planet1=p1, planet2=p2,
                    sign1_index=pos1.sign_index, sign2_index=pos2.sign_index,
                    house1=ph.get(p1,0), house2=ph.get(p2,0),
                    strength=strength,
                ))
