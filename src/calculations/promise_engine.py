...
def compute_house_promise(chart, house: int) -> PromiseLevel:
    ...
    if mals:
        key_factors.append(f"Malefics in house: {mals}")
    ...
    return PromiseLevel(
        house=house,
        natal_score=round(score, 3),
        promise_present=present,
...
def compute_full_promise(
    ...
    for h in range(1, 13):
        ...
        dasha_activated = active_md_lord in {
            lord,
            *[p for p, hp in ph.items() if hp == h],
        } or active_ad_lord in {lord, *[p for p, hp in ph.items() if hp == h]}
