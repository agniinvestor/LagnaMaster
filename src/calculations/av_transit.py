...
def planet_transit_quality(planet: str, transit_si: int, natal_chart) -> PlanetTransitQuality:
    ...
    if rekhas >= 6:   quality, comment = "Excellent", "High rekhas — very favorable transit"
    elif rekhas == 5: quality, comment = "Good",      "Average-high — favorable transit"
    elif rekhas == 4: quality, comment = "Average",   "Neutral transit"
    elif rekhas == 3: quality, comment = "Unfavorable","Low rekhas — challenging transit"
    else:             quality, comment = "Malefic",   "Very low rekhas — avoid major decisions"

    return PlanetTransitQuality(
        planet=planet, transit_sign_index=transit_si,
        transit_sign=_SIGNS[transit_si % 12],
        rekhas=rekhas, quality=quality, commentary=comment,
    )


def compute_transit_av_score(natal_chart, transit_date: date | None = None,
    ...
    if transit_longitudes is None:
        try:
            ...
            t_chart = compute_chart(
                year=transit_date.year, month=transit_date.month,
                day=transit_date.day, hour=12.0,
            ...
            transit_longitudes = {p: t_chart.planets[p].longitude
                                   for p in planets_7 if p in t_chart.planets}
        except Exception:
            transit_longitudes = {}
    ...
    active_favorable = [p for p, q in planet_qualities.items()
                        if q.quality in {"Excellent","Good"}]
