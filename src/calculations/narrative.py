...
def generate_narrative(lpi_result, chart, dashas=None, on_date=None) -> NarrativeReport:
    ...
    for h, hlpi in lpi_result.houses.items():
        ...
        if fi >= 3.0:
            ...
            quality = (
                "under pressure — more affliction than support; requires attention"
            )
        else:
            quality = "significantly challenged across multiple axes"

        conf_str = {
            "High": "High-confidence signal — inter-axis agreement is strong",
            "Med": "Medium confidence — some axis divergence present",
            "Low": "Low confidence — significant divergence; nuanced reading needed",
        }.get(conf, "")

        text = (
            f"H{h} {hlpi.domain} ({_RAG_LABEL.get(rag, 'mixed')}): {quality}. "
    ...
    for dom, dom_avg in lpi_result.domain_balance.items():
        if dom_avg >= 1.0:
            qual = "generally supported"
        elif dom_avg >= -1.0:
            qual = "mixed"
        else:
            qual = "under collective pressure"
        domain_summaries[dom] = f"{dom} axis ({qual}, avg {dom_avg:+.2f})"

    overall = lpi_result.overall_index
    if overall >= 2.0:
        ov_text = "Chart shows overall positive momentum currently."
    elif overall >= 0.0:
        ov_text = "Chart is in a mixed period — pockets of strength and challenge."
    else:
        ov_text = "Chart is under collective pressure — key life areas need attention."
