...
_DOMAIN_WEIGHTS = {
    "career": {
        "D1": 0.20, "Chandra": 0.05, "Surya": 0.10,
        "D9": 0.10, "D10": 0.35, "Dasha": 0.15, "Gochar": 0.05,
        "primary_house": 10,
        "rationale": "D10 dominates career. PVRNR: '10th from AL in D-10 shows perceptions about career.'"
    },
    "marriage": {
        "D1": 0.20, "Chandra": 0.10, "Surya": 0.05,
        "D9": 0.35, "D10": 0.05, "Dasha": 0.15, "Gochar": 0.10,
        "primary_house": 7,
        "rationale": "D9 dominates dharmic unions. PVRNR: 'D-9 is the best chart for marriage.'"
    },
    "mind_psychology": {
        "D1": 0.20, "Chandra": 0.40, "Surya": 0.10,
        "D9": 0.10, "D10": 0.05, "Dasha": 0.10, "Gochar": 0.05,
        "primary_house": 4,
        "rationale": "Chandra Lagna dominates emotional/psychological matters."
    },
    "wealth": {
        "D1": 0.35, "Chandra": 0.10, "Surya": 0.10,
        "D9": 0.10, "D10": 0.10, "Dasha": 0.15, "Gochar": 0.10,
        "primary_house": 2,
        "rationale": "D1 H2 and H11 primary for wealth accumulation."
    },
    "health_longevity": {
        "D1": 0.45, "Chandra": 0.15, "Surya": 0.10,
        "D9": 0.10, "D10": 0.05, "Dasha": 0.10, "Gochar": 0.05,
        "primary_house": 8,
        "rationale": "D1 dominates physical vitality. H1/H8 are the life axis."
    },
    "spirituality": {
        "D1": 0.15, "Chandra": 0.10, "Surya": 0.05,
        "D9": 0.45, "D10": 0.05, "Dasha": 0.15, "Gochar": 0.05,
        "primary_house": 9,
        "rationale": "D9 = dharma/soul chart. PVRNR: 'D-20 for religious activities.'"
    },
    "children": {
        "D1": 0.25, "Chandra": 0.10, "Surya": 0.10,
        "D9": 0.20, "D10": 0.05, "Dasha": 0.20, "Gochar": 0.10,
        "primary_house": 5,
        "rationale": "D7 (saptamsha) ideal but not in LPI axes. D9 secondary."
    },
    "default": {
        "D1": 0.35, "Chandra": 0.15, "Surya": 0.10,
        "D9": 0.15, "D10": 0.10, "Dasha": 0.10, "Gochar": 0.05,
        "primary_house": None,
...
def compute_domain_lpi(chart, dashas=None, on_date=None,
                        domain: str = "default") -> DomainLPIResult:
    """Compute LPI with domain-specific axis weights."""
    from datetime import date as _date
    if on_date is None: on_date = _date.today()
    ...
    domain_score = round(
        0.4 * primary_score + 0.6 * (sum(house_scores.values()) / 12),
        3
    )

    return DomainLPIResult(
        domain=domain, primary_house=primary_house,
        weights_used={k: v for k, v in weights.items()
                      if k not in ("primary_house","rationale")},
        house_scores=house_scores, top_houses=top, weak_houses=weak,
        domain_score=domain_score, rationale=weights.get("rationale",""),
    )
