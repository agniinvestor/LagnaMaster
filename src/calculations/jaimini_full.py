...
def detect_jaimini_yogas(chart) -> list[JaiminiYoga]:
    ...
    try:
        ...
        h9_sign = (kk_si + 8) % 12
        h5_planets = [
            p for p in chart.planets if chart.planets[p].sign_index == h5_sign
        ]
        h9_planets = [
            p for p in chart.planets if chart.planets[p].sign_index == h9_sign
        ]
        benefics_5_9 = [
            p
    ...
    if ul_lord_pos:
        ul_good = (
            ul_lord_pos.sign_index in _OWN.get(ul_lord, set())
            or _EXALT_SI.get(ul_lord) == ul_lord_pos.sign_index
        )
    yogas.append(
        JaiminiYoga(
            "Upapada Lord Strong",
    ...
    try:
        ...
        if hasattr(_karakas, "items"):
            ...
            _ak = (
                next((pl for pl, r in _karakas if r == "AK"), "Sun")
                if _karakas
                else "Sun"
            )
...
def compute_jaimini_longevity(chart) -> JaiminiLongevity:
    ...
    odd_planets = [
        p
        for p in chart.planets
        if ph.get(p, 0) in odd_houses and p not in {"Rahu", "Ketu"}
    ]
    ...
    rudra = (
        lord8
        if _HOUSE_STRENGTH.get(h8_lord_house, 1)
        >= _HOUSE_STRENGTH.get(h12_lord_house, 1)
        else lord12
    )

    # Span estimate from combination
    strong_count = sum(
        1 for p in [brahma, maheshvara, rudra] if ph.get(p, 0) in {1, 4, 7, 10}
    )
    if strong_count >= 2:
    ...
    return JaiminiLongevity(
        brahma=brahma,
        maheshvara=maheshvara,
        rudra=rudra,
        span=span,
        years_estimate=years,
    )
