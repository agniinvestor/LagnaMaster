...
def compare_scenarios(
    ...
    dashas=None, on_date=None, school="parashari",
) -> list[ScenarioResult]:
    ...
    for label, overrides in overrides_list:
        ...
        d1_delta = {h: round(sc_scores.d1_scores[h] - base.d1_scores[h], 3)
                    for h in range(1, 13)}
        lpi_delta = {}
        if base.lpi and sc_scores.lpi:
            lpi_delta = {h: round(sc_scores.lpi.houses[h].full_index
                                  - base.lpi.houses[h].full_index, 3)
                         for h in range(1, 13)}
        best_h = max(d1_delta, key=d1_delta.get)
        worst_h = min(d1_delta, key=d1_delta.get)
        summary = (f"Scenario '{label}': best gain H{best_h} ({d1_delta[best_h]:+.2f}), "
                   f"worst H{worst_h} ({d1_delta[worst_h]:+.2f})")
        results.append(ScenarioResult(
            label=label, d1_delta=d1_delta,
            lpi_delta=lpi_delta, summary=summary,
        ))
    return results
