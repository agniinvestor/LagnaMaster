...
def _build_wheel(reference_si: int, reference: str) -> SudarshanWheel:
    ...
    return SudarshanWheel(reference=reference, reference_sign=reference_si,
...
def compute_dasha_pravesh(
    ...
    if planet not in natal_chart.planets:
        return DashaPraveshResult(
            planet=planet, query_year=query_year,
    ...
    try:
        ...
        _PERIODS = {
            "Sun": 1.0, "Moon": 0.0748, "Mars": 1.881,
            "Mercury": 0.241, "Jupiter": 11.86, "Venus": 0.615,
            "Saturn": 29.46,
        }
