...
def structural_vulnerability(chart) -> tuple[float, list[str]]:
    ...
    from src.calculations.avastha import compute_lajjitadi, compute_deeptadi
    ...
    if moon_pos and sat_pos:
        if moon_pos.sign_index == sat_pos.sign_index:
            score += 2.0; drivers.append("Saturn conjunct Moon (natal)")
        ...
        for aspect_offset in [3, 7, 10]:
            if (sat_house - 1 + aspect_offset - 1) % 12 + 1 == moon_house:
                score += 1.0; drivers.append(f"Saturn aspects Moon ({aspect_offset}th aspect)")
                break
    ...
    if badhaka_lord in hmap.planet_house:
        ...
        if bl_house in {1, 4, 7, 10}:
            score += 1.5; drivers.append(f"Badhaka lord {badhaka_lord} in kendra H{bl_house}")
        elif bl_house in {1}:
            score += 2.0; drivers.append(f"Badhaka lord {badhaka_lord} in lagna")
    ...
    for planet in roles.functional_malefics:
        if planet in hmap.planet_house:
            ...
            if h in {1, 4, 7, 10}:
                score += 0.5; drivers.append(f"{planet} (func. malefic) in kendra H{h}")
    ...
    if d8_lord in hmap.planet_house:
        if hmap.planet_house[d8_lord] == 1:
            score += 1.5; drivers.append("H8 lord in H1 (transformation pressure)")

    # Lajjitadi state of 5th lord
    laj = compute_lajjitadi(chart)
    if laj.state == "Lajjita":
        score += 2.0; drivers.append(f"5th lord {laj.fifth_lord} Lajjita: {', '.join(laj.triggers)}")
    elif laj.state in {"Kshobhita", "Kshudhita"}:
        score += 1.0; drivers.append(f"5th lord {laj.fifth_lord} {laj.state}")
...
def dasha_activation_weight(chart, dashas: list, on_date: date) -> tuple[float, str]:
    ...
    for lord, kind in [(md.lord, "MD"), (ad.lord, "AD")]:
        ...
        if lord in roles.functional_malefics:
            weight += 0.25; notes.append(f"{lord} {kind} is func. malefic")

        # Dasha lord is badhaka lord
        if lord == roles.badhaka_lord:
            weight += 0.4; notes.append(f"{lord} {kind} is badhaka lord")

        # Dasha lord is maraka lord
        if lord in roles.maraka_lords:
            weight += 0.2; notes.append(f"{lord} {kind} is maraka lord")

        # Dasha lord placed in dusthana
        if lord in hmap.planet_house and hmap.planet_house[lord] in {6, 8, 12}:
            weight += 0.25; notes.append(f"{lord} {kind} in dusthana H{hmap.planet_house[lord]}")

        # Yogakaraka running — reduces weight
        if lord in roles.yogakarakas:
            weight -= 0.3; notes.append(f"{lord} {kind} is yogakaraka (protective)")

    note = f"{md.lord} MD / {ad.lord} AD: {'; '.join(notes)}" if notes else f"{md.lord} MD / {ad.lord} AD (neutral)"
...
def transit_load(chart, on_date: date) -> tuple[float, str]:
    ...
    if gochara.sade_sati:
        phase_weight = {"Peak": 0.6, "Rising": 0.3, "Setting": 0.2}.get(gochara.sade_sati_phase, 0.2)
        load += phase_weight
    ...
    if sat_transit and sat_transit.sign_index == lagna_si:
        load += 0.4; notes.append("Saturn transiting Lagna")
    ...
    if rahu_transit:
        if rahu_transit.sign_index in {moon_natal_si, lagna_si}:
            load += 0.3; notes.append("Rahu transiting Moon/Lagna sign")
    ...
    if mars_transit and mars_transit.sign_index == moon_natal_si:
        load += 0.2; notes.append("Mars transiting natal Moon sign")
    ...
    for si, planets in transit_signs.items():
        if len(planets) >= 2:
            load += 0.3; notes.append(f"Malefic cluster ({', '.join(planets)}) in {_sign_name(si)}")
...
def resilience_factor(chart, dashas: list, on_date: date) -> tuple[float, str]:
    ...
    try:
        ...
        if jup_sb.total > 300:
            resilience += 0.3; notes.append(f"Strong Jupiter ({jup_sb.total:.0f} Virupas)")
        elif jup_sb.total < 150:
            resilience -= 0.2; notes.append(f"Weak Jupiter ({jup_sb.total:.0f} Virupas)")
    except Exception:
        pass
    ...
    try:
        ...
        if jup_tr:
            ...
            if dist in {1, 4, 7, 10}:
                resilience += 0.3; notes.append("Jupiter in kendra from natal Moon (transit)")
    except Exception:
        pass
...
def compute_pressure_index(chart, dashas: list, on_date: Optional[date] = None) -> PressurePoint:
