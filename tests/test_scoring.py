...
class TestScoringEngine:
    ...
    def test_26_rules_per_house(self, india_chart):
        from src.scoring import score_chart

        result = score_chart(india_chart)
        for h, hs in result.houses.items():
            assert len(hs.rules) == 26, f"H{h} has {len(hs.rules)} rules, expected 26"
    ...
    def test_ratings_are_valid(self, india_chart):
        ...
        for h, hs in result.houses.items():
            assert hs.rating in valid_ratings, f"H{h} has invalid rating '{hs.rating}'"
