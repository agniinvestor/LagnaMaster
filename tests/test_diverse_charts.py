...
class TestDignityRules:
    ...
    def test_all_12_lagnas_functional_dignity(self):
        ...
        for lagna_si in range(12):
            ...
            ll = _SL[lagna_si]
            assert fc[ll].is_functional_benefic, (
                f"Lagna {lagna_si}: lord {ll} not benefic"
            )
...
class TestYogaDetection:
    ...
    def test_raj_yoga_lords_in_same_sign(self):
        ...
        assert moon_si == mars_si, (
            f"Moon({moon_si}) and Mars({mars_si}) not in same sign"
        )
...
class TestTransitQuality:
    ...
    def test_64th_navamsha_formula(self):
        ...
        {"lagna": 37.73, "planets": {"Moon": MagicMock(longitude=93.98)}}
        chart = MagicMock()
        chart.lagna = 37.73
