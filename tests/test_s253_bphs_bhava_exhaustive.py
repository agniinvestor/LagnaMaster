...
def test_all_12_houses_covered():
    ...
    for h, suffix in ordinals.items():
        house_rules = [r for r in BPHS_BHAVA_EXHAUSTIVE_REGISTRY.all() if suffix in r.tags]
...
def test_marana_karaka_rules():
    ...
    marana = [
        ...
        if any("marana" in t for t in r.tags)
    ]
...
def test_maraka_rules():
    from src.corpus.bphs_bhava_exhaustive import BPHS_BHAVA_EXHAUSTIVE_REGISTRY
    maraka = [r for r in BPHS_BHAVA_EXHAUSTIVE_REGISTRY.all() if "maraka" in r.tags]
    assert len(maraka) >= 2
...
def test_body_anatomy_rule():
    ...
    assert rule is not None
    assert "body_anatomy" in rule.tags
...
def test_longevity_classification():
    ...
    assert rule is not None
    assert "alpayu" in rule.tags
    assert "purnayu" in rule.tags
