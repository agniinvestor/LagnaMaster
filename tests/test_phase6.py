...
INDIA = dict(year=1947,month=8,day=15,hour=0.0,lat=28.6139,lon=77.2090,tz_offset=5.5,ayanamsha="lahiri")
...
class TestKPFull:
    def test_kp_chain_structure(self, chart):
        ...
        assert chain.sign_lord in ["Sun","Moon","Mars","Mercury","Jupiter","Venus","Saturn"]
        assert chain.nak_lord in ["Sun","Moon","Mars","Mercury","Jupiter","Venus","Saturn","Rahu","Ketu"]
        assert chain.sub_lord in ["Sun","Moon","Mars","Mercury","Jupiter","Venus","Saturn","Rahu","Ketu"]
    ...
    def test_kp_event_promise(self, chart):
        ...
        assert p.sub_lord in ["Sun","Moon","Mars","Mercury","Jupiter","Venus","Saturn","Rahu","Ketu"]
...
class TestSpecialLagnas:
    def test_returns_all_five(self, chart):
        ...
        assert s.hora_lagna_sign in ["Aries","Taurus","Gemini","Cancer","Leo","Virgo",
                                      "Libra","Scorpio","Sagittarius","Capricorn","Aquarius","Pisces"]
        assert s.ghati_lagna_sign in ["Aries","Taurus","Gemini","Cancer","Leo","Virgo",
                                       "Libra","Scorpio","Sagittarius","Capricorn","Aquarius","Pisces"]

    def test_indices_in_range(self, chart):
        from src.calculations.special_lagnas import compute_special_lagnas
        s = compute_special_lagnas(chart)
        for idx in [s.hora_lagna_index, s.ghati_lagna_index,
                    s.sree_lagna_index, s.indu_lagna_index, s.pranapada_index]:
            assert 0 <= idx <= 11
...
class TestJaiminiFull:
    ...
    def test_jaimini_longevity_structure(self, chart):
        ...
        assert r.span in {"Short","Medium","Long"}
        assert r.brahma in ["Sun","Moon","Mars","Mercury","Jupiter","Venus","Saturn","Rahu","Ketu"]
    ...
    def test_ak_amk_yoga(self, chart):
        from src.calculations.jaimini_full import detect_jaimini_yogas
        r = detect_jaimini_yogas(chart)
        ak_yoga = next((y for y in r if "AK+AmK" in y.name or "Atmakaraka" in y.name), None)
...
class TestEmpirica:
    def test_init_and_record(self, tmp_path):
        from src.calculations.empirica import (
            init_empirica_db, record_event, EmpiricalEvent, get_events
        )
        db = tmp_path / "test_empirica.db"
        init_empirica_db(db)
        evt = EmpiricalEvent(
            chart_id="CHART_001", event_date="1947-08-15",
            event_type="Career", house_primary=10,
            event_description="Independence", manifested=1, confidence=3,
        )
    ...
    def test_accuracy_with_events(self, tmp_path):
        from src.calculations.empirica import (
            init_empirica_db, record_event, compute_accuracy, EmpiricalEvent
        )
        db = tmp_path / "acc.db"
        init_empirica_db(db)
        for i, manifested in enumerate([1,1,0,1]):
            record_event(EmpiricalEvent(
                chart_id="C1", event_date=f"2020-0{i+1}-01",
                event_type="Career", house_primary=10,
                manifested=manifested, r04_fired=1 if i < 3 else 0,
            ), db)
        r = compute_accuracy(db)
    ...
    def test_event_type_validation(self, tmp_path):
        ...
        from src.calculations.empirica import (
            init_empirica_db, record_event, get_events, EmpiricalEvent
        )
        db = tmp_path / "type.db"
        init_empirica_db(db)
        evt = EmpiricalEvent(
            chart_id="C2", event_date="2020-01-01",
            event_type="UnknownType", house_primary=5,
        )
        record_event(evt, db)
    ...
    def test_three_seed_events_schema(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            init_empirica_db(db)
            seeds = [
                EmpiricalEvent("INDIA_1947","1971-12-16","Career",10,
                    "1971 war victory",manifested=1,confidence=3,
                    active_mahadasha="Saturn",d1_score_at_event=3.5),
                EmpiricalEvent("INDIA_1947","1991-07-24","Finance",11,
                    "Economic liberalisation",manifested=1,confidence=2,
                    active_mahadasha="Jupiter",d1_score_at_event=2.0),
                EmpiricalEvent("INDIA_1947","2001-12-13","Conflicts",6,
                    "Parliament attack",manifested=1,confidence=2,
                    active_mahadasha="Saturn",d1_score_at_event=-2.5),
            ]
            for s in seeds:
                record_event(s, db)
