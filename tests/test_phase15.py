...
class TestPanchanga:
    ...
    def test_tithi_calculation(self):
        ...
        assert p.tithi in {
            15,
            16,
        }  # 180° is boundary between Purnima and Krishna Pratipada

    def test_nakshatra_range(self, chart):
        from src.calculations.panchanga import compute_panchanga

        p = compute_panchanga(
            chart.planets["Sun"].longitude, chart.planets["Moon"].longitude
        )
...
class TestAshtottariDasha:
    ...
    def test_total_years(self, chart):
        from src.calculations.ashtottari_dasha import (
