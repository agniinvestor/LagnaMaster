...
class TestReg:
    ...
    def test_email_lower(self, tmp_path):
        a, db = _f(tmp_path)
        u = a.register_user("e", "E@X.COM", "password123", path=db)
...
class TestAuth:
    ...
    def test_deactivated(self, tmp_path):
        a, db = _f(tmp_path)
        u = a.register_user("i", "i@x.com", "password123", path=db)
...
class TestTok:
    ...
    def test_cross_reject(self, tmp_path):
        ...
        with pytest.raises(ValueError):
            a.verify_refresh_token(p.access_token)
    ...
    def test_tamper(self, tmp_path):
        ...
        p = a.create_token_pair(u.id)
        with pytest.raises(ValueError):
            a.verify_access_token(p.access_token[:-4] + "XXXX")
...
class TestRouter:
    ...
    def test_register_201(self, client):
        assert (
            client.post(
                "/auth/register",
                json={"username": "sam", "email": "s@x.com", "password": "password123"},
            ).status_code
            == 201
        )
    ...
    def test_conflict_409(self, client):
        client.post(
            "/auth/register",
            json={"username": "tim", "email": "t@x.com", "password": "password123"},
        )
        assert (
            client.post(
                "/auth/register",
    ...
    def test_login_tokens(self, client):
        client.post(
            "/auth/register",
            json={"username": "uma", "email": "u@x.com", "password": "password123"},
        )
        r = client.post(
            "/auth/login", json={"username": "uma", "password": "password123"}
        )
    ...
    def test_login_wrong_401(self, client):
        client.post(
            "/auth/register",
            json={"username": "vic", "email": "v@x.com", "password": "password123"},
        )
        assert (
            client.post(
                "/auth/login", json={"username": "vic", "password": "wrong"}
            ).status_code
            == 401
        )
    ...
    def test_me(self, client):
        client.post(
            "/auth/register",
            json={"username": "wes", "email": "w@x.com", "password": "password123"},
        )
        tok = client.post(
            "/auth/login", json={"username": "wes", "password": "password123"}
        ).json()["access_token"]
    ...
    def test_refresh(self, client):
        client.post(
            "/auth/register",
            json={"username": "xan", "email": "x@x.com", "password": "password123"},
        )
        rt = client.post(
            "/auth/login", json={"username": "xan", "password": "password123"}
        ).json()["refresh_token"]
