...
def test_all_9_planets_covered():
    ...
    for planet in planets:
        planet_rules = [r for r in BPHS_GRAHA_CHARACTERISTICS_REGISTRY.all() if planet in r.keyword_tags]
...
def test_special_aspects_rule():
    ...
    assert "saturn_3_10" in rule.keyword_tags
    assert "jupiter_5_9" in rule.keyword_tags
    assert "mars_4_8" in rule.keyword_tags
...
def test_combustion_degrees():
    ...
    assert rule is not None
    assert "combustion" in rule.keyword_tags
...
def test_vimshottari_periods():
    ...
    assert rule is not None
    assert "venus_20" in rule.keyword_tags
    assert "saturn_19" in rule.keyword_tags
...
def test_naisargika_bala():
    ...
    assert rule is not None
    assert "sun_60" in rule.keyword_tags
    assert "saturn_8.57" in rule.keyword_tags
...
def test_maturation_ages():
    ...
    assert rule is not None
    assert "saturn_36" in rule.keyword_tags
    assert "jupiter_16" in rule.keyword_tags
...
def test_medical_astrology_covered():
    from src.corpus.bphs_graha_characteristics import BPHS_GRAHA_CHARACTERISTICS_REGISTRY
    medical = [r for r in BPHS_GRAHA_CHARACTERISTICS_REGISTRY.all() if "medical_astrology" in r.keyword_tags]
    assert len(medical) >= 8


def test_gems_and_metals():
    from src.corpus.bphs_graha_characteristics import BPHS_GRAHA_CHARACTERISTICS_REGISTRY
    gems = [r for r in BPHS_GRAHA_CHARACTERISTICS_REGISTRY.all() if "gem_ruby" in r.keyword_tags or "gem_pearl" in r.keyword_tags
            or "gem_emerald" in r.keyword_tags or "gem_yellow_sapphire" in r.keyword_tags or "gem_diamond" in r.keyword_tags
            or "gem_blue_sapphire" in r.keyword_tags or "gem_hessonite_catseye" in r.keyword_tags or "gem_red_coral" in r.keyword_tags]
