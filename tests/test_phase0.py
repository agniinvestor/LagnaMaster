...
class TestAVShodhana:
    ...
    def test_av_fixed_totals_pre_shodhana(self):
        ...
        for planet, expected in FIXED_TOTALS_RAW.items():
            ...
            assert abs(raw_total - expected) <= 4, (
                f"{planet}: raw={raw_total} expected≈{expected}"
            )
