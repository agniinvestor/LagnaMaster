...
def test_all_12_rashis_covered():
    ...
    for rashi in rashis:
        rules = [r for r in BRIHAT_JATAKA_EXHAUSTIVE_REGISTRY.all() if rashi in r.keyword_tags]
...
def test_bj_tag_on_all_rules():
    from src.corpus.brihat_jataka_exhaustive import BRIHAT_JATAKA_EXHAUSTIVE_REGISTRY
    for rule in BRIHAT_JATAKA_EXHAUSTIVE_REGISTRY.all():
        assert "bj" in rule.keyword_tags, f"{rule.rule_id} missing 'bj' tag"
...
def test_navamsha_rule():
    ...
    assert rule is not None
    assert "navamsha_d9" in rule.keyword_tags or "navamsha" in rule.keyword_tags or "d9" in rule.keyword_tags
...
def test_jupiter_transit_rule():
    ...
    assert rule is not None
    assert "transit" in rule.keyword_tags
...
def test_sade_sati_rule():
    ...
    assert rule is not None
    assert "sade_sati" in rule.keyword_tags or "saturn_transit" in rule.keyword_tags


def test_avastha_rules():
    from src.corpus.brihat_jataka_exhaustive import BRIHAT_JATAKA_EXHAUSTIVE_REGISTRY
    avastha = [r for r in BRIHAT_JATAKA_EXHAUSTIVE_REGISTRY.all()
               if any("avastha" in t for t in r.keyword_tags)]
    assert len(avastha) >= 1


def test_longevity_rules():
    from src.corpus.brihat_jataka_exhaustive import BRIHAT_JATAKA_EXHAUSTIVE_REGISTRY
    longevity = [r for r in BRIHAT_JATAKA_EXHAUSTIVE_REGISTRY.all() if "longevity" in r.keyword_tags]
    assert len(longevity) >= 3


def test_nakshatra_rules():
    from src.corpus.brihat_jataka_exhaustive import BRIHAT_JATAKA_EXHAUSTIVE_REGISTRY
    nakshatra = [r for r in BRIHAT_JATAKA_EXHAUSTIVE_REGISTRY.all() if "nakshatra" in r.keyword_tags]
    assert len(nakshatra) >= 5


def test_yoga_rules_covered():
    from src.corpus.brihat_jataka_exhaustive import BRIHAT_JATAKA_EXHAUSTIVE_REGISTRY
    yogas = [r for r in BRIHAT_JATAKA_EXHAUSTIVE_REGISTRY.all() if "yoga" in r.keyword_tags]
    assert len(yogas) >= 5


def test_dasha_rules_covered():
    from src.corpus.brihat_jataka_exhaustive import BRIHAT_JATAKA_EXHAUSTIVE_REGISTRY
    dasha = [r for r in BRIHAT_JATAKA_EXHAUSTIVE_REGISTRY.all() if "dasha" in r.keyword_tags]
    assert len(dasha) >= 3
