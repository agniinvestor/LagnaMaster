...
def test_dusthana_houses():
    ...
    assert "dusthana" in rule.tags
    assert "6_8_12" in rule.tags
...
def test_trikona_houses():
    ...
    assert rule is not None
    assert "trikona" in rule.tags
    assert "1_5_9" in rule.tags
...
def test_bhavat_bhavam():
    ...
    assert "5th_from_5th" in rule.tags


def test_maraka_principle():
    from src.corpus.bhava_phala_rules import BHAVA_PHALA_RULES_REGISTRY
    maraka_rules = [r for r in BHAVA_PHALA_RULES_REGISTRY.all() if "maraka" in r.tags]
...
def test_health_body_parts():
    ...
    assert rule is not None
    assert "12_houses_anatomy" in rule.tags
    assert "health_body_parts" in rule.tags
