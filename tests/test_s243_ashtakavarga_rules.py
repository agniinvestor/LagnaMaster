...
def test_fundamental_structure():
    ...
    assert "sarvashtakavarga" in rule.tags
...
def test_trikona_shodhana():
    ...
    assert rule is not None
    assert "trikona_shodhana" in rule.tags
    assert "reduction" in rule.tags
...
def test_ekadhipatya_shodhana():
    ...
    assert rule is not None
    assert "ekadhipatya_shodhana" in rule.tags
    assert "mercury_venus_saturn" in rule.tags
...
def test_saturn_bav_sade_sati():
    ...
    assert rule is not None
    assert "saturn_bav" in rule.tags
    assert "sade_sati" in rule.tags


def test_kakshya_timing():
    from src.corpus.ashtakavarga_rules import ASHTAKAVARGA_RULES_REGISTRY
    kakshya_rules = [r for r in ASHTAKAVARGA_RULES_REGISTRY.all() if "kakshya" in r.tags]
...
def test_compatibility_rule():
    ...
    assert rule is not None
    assert "compatibility" in rule.tags
    assert "synastry" in rule.tags
