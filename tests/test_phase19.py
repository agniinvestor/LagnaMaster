...
class TestDoubleTransit:
    ...
    def test_signal_valid(self, chart):
        ...
        assert "double" in r.signal.lower() or "partial" in r.signal.lower() or "not" in r.signal.lower()
...
class TestUpapadaLagna:
    def test_computes_ul(self, chart):
        ...
        assert r.ul_sign in ["Aries","Taurus","Gemini","Cancer","Leo","Virgo",
    ...
    def test_longevity_valid(self, chart):
        from src.calculations.upapada_lagna import compute_upapada
        r = compute_upapada(chart)
        assert "lasting" in r.marriage_longevity.lower() or \
               "challenge" in r.marriage_longevity.lower() or \
               "separation" in r.marriage_longevity.lower()
...
class TestKalaSarpa:
    ...
    def test_disclaimer_present(self, chart):
        ...
        assert "not found" in r.classical_disclaimer.lower() or \
               "convention" in r.classical_disclaimer.lower()
...
class TestRulePlugin:
    def test_register_and_apply(self, chart):
        from src.calculations.rule_plugin import register_yoga, apply_all_plugins, clear_plugins
        clear_plugins()
