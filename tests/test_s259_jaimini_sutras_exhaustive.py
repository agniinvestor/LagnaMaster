...
def test_jmx_tag_on_all_rules():
    from src.corpus.jaimini_sutras_exhaustive import JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY
    for rule in JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY.all():
        assert "jmx" in rule.tags, f"{rule.rule_id} missing 'jmx' tag"


def test_jaimini_tag_on_all_rules():
    from src.corpus.jaimini_sutras_exhaustive import JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY
    for rule in JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY.all():
        assert "jaimini" in rule.tags, f"{rule.rule_id} missing 'jaimini' tag"


def test_chara_karaka_rules():
    from src.corpus.jaimini_sutras_exhaustive import JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY
    ck = [r for r in JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY.all() if "chara_karaka" in r.tags]
    assert len(ck) >= 5


def test_atmakaraka_rules():
    from src.corpus.jaimini_sutras_exhaustive import JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY
    ak = [r for r in JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY.all() if "atmakaraka" in r.tags]
    assert len(ak) >= 10


def test_rashi_drishti_rules():
    from src.corpus.jaimini_sutras_exhaustive import JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY
    rd = [r for r in JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY.all() if "rashi_drishti" in r.tags]
    assert len(rd) >= 8


def test_arudha_rules():
    from src.corpus.jaimini_sutras_exhaustive import JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY
    arudha = [r for r in JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY.all() if "arudha" in r.tags]
    assert len(arudha) >= 8


def test_upapada_rules():
    from src.corpus.jaimini_sutras_exhaustive import JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY
    ul = [r for r in JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY.all() if "upapada" in r.tags]
    assert len(ul) >= 4


def test_karakamsha_rules():
    from src.corpus.jaimini_sutras_exhaustive import JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY
    km = [r for r in JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY.all() if "karakamsha" in r.tags]
    assert len(km) >= 10


def test_chara_dasha_rules():
    from src.corpus.jaimini_sutras_exhaustive import JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY
    cd = [r for r in JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY.all() if "chara_dasha" in r.tags]
    assert len(cd) >= 8


def test_raja_yoga_rules():
    from src.corpus.jaimini_sutras_exhaustive import JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY
    raja = [r for r in JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY.all() if "raja_yoga" in r.tags]
    assert len(raja) >= 5


def test_moksha_rules():
    from src.corpus.jaimini_sutras_exhaustive import JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY
    moksha = [r for r in JAIMINI_SUTRAS_EXHAUSTIVE_REGISTRY.all() if "moksha" in r.tags]
    assert len(moksha) >= 3
...
def test_jmx036_chara_dasha():
    ...
    assert rule is not None
    assert "chara_dasha" in rule.tags
...
def test_jmx087_ishta_devata():
    ...
    assert rule is not None
    assert "ishta_devata" in rule.tags
