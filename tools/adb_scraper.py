...
def _parse_birth_data_from_html(html: str, slug: str) -> Optional[ADBRecord]:
    ...
    re.search(
        r"(?:born|Birth[^<]*|bday)[^<]*?(\w+ \d{1,2},?\s+\d{4})", html, re.IGNORECASE
    )
    ...
    if dt_match:
        ...
        return ADBRecord(
            ...
            birth_lat=lat or 20.0,
            birth_lon=lon or 77.0,
            birth_place="Unknown",
...
def compute_chart_and_scores(fixture: dict) -> dict:
    ...
    try:
        ...
        fixture["scores"] = {
            str(h): round(float(hs.final_score), 4) for h, hs in result.houses.items()
        }
...
def print_report(fixtures: list[dict]):
    ...
    print(f"\n{'=' * 60}")
    print("ADB Fixture Library — LagnaMaster")
    print(f"{'=' * 60}")
    ...
    for f in fixtures:
        ...
        print(
            f"  {f['name']:<30} {f.get('rodden_rating', '?'):3}  "
            f"{bd.get('year', '?')}  "
            f"Best: H{top}"
        )
...
MANUAL_AA_FIXTURES = {
    ...
    "nehru_jawaharlal": {
        ...
        "birth_data": {
            ...
            "lat": 25.4358,
            "lon": 81.8463,  # Allahabad (Prayagraj)
            "tz_offset": 5.5,
            "ayanamsha": "lahiri",
        },
    ...
    "lincoln_abraham": {
        ...
        "birth_data": {
            ...
            "hour": 7.75,
            "lat": 37.5408,
            "lon": -85.7267,  # Hodgenville, Kentucky
            "tz_offset": -6.0,
            "ayanamsha": "lahiri",
        },
    ...
    "mandela_nelson": {
        ...
        "birth_data": {
            ...
            "hour": 14.75,
            "lat": -31.5667,
            "lon": 28.7833,  # Mvezo, Eastern Cape, South Africa
            "tz_offset": 2.0,
            "ayanamsha": "lahiri",
        },
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--compute", action="store_true", help="Compute charts + scores"
    )
    ...
    if args.fetch:
        ...
        for slug, name, _, notes in CURATED_PERSONS:
            if fetched >= args.limit:
                break
            path = os.path.join(
                FIXTURES_DIR, slug.replace(",", "").replace(" ", "_") + ".json"
            )
